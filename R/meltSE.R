...
#' @param rowlDat.columns The rowData columns to include (none included by default). Use
...
#' @export
meltSE <- function(x, genes, assayName=NULL, colDat.columns=NULL, rowDat.columns=NA){
  ...
  a <- lapply(a, FUN=function(x) x[genes,,drop=F])
