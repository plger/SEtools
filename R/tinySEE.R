      ...
      dashboardBody(
        tabItems(
          tabItem("tab_fileinput",
            box(width=7,
                fileInput( "file", "Choose SE .rds file", multiple = FALSE, accept=c(".rds")),
                withSpinner(textOutput("fileout")),
                withSpinner(verbatimTextOutput("fileout2")) )
          ),
          tabItem("tab_samples", box(width=12, tags$div(style="overflow: auto;",
    	withSpinner(DTOutput("samples"))))),
          tabItem("tab_features", box(width=12, tags$div(style="overflow: auto;",
    	withSpinner(DTOutput("features"))),
    	shiny::actionButton("transfer_geneSel", "Transfer filtered genes (max 500) to the heatmap"))),
          ...
          tabItem("tab_heatmap",
               box( width=12, title="Genes", collapsible=TRUE,
                    textAreaInput('input_genes','Genes to plot', width="90%", rows=7,
...
.tinySEE_server <- function(se=NULL, maxSize=50*1024^2){
    ...
    shinyServer(function(input, output, session) {
        ...
        SEinit <- function(x){
            ...
            updateSelectizeInput(session, "gene_input", choices=sort(unique(row.names(x))), server=T)
            colvars <- colnames(colData(x))
            updateSelectInput(session, "assay_input", choices=assayNames(x), selected=getDef(x, "assay"))
            updateSelectInput(session, "assay_input2", choices=assayNames(x), selected=getDef(x, "assay"))
            updateSelectizeInput(session, "hm_order", choices=colvars)
            updateSelectizeInput(session, "hm_anno", choices=colvars, selected=getDef(x, c("groupvar","colvar","gridvar")))
            updateSelectizeInput(session, "hm_gaps", choices=colvars, selected=getDef(x, "gridvar"))
            updateSelectInput(session, "select_groupvar", choices=colvars, selected=getDef(x, "groupvar"))
            updateSelectInput(session, "select_colorvar", choices=colvars, selected=getDef(x, "colvar"))
            updateSelectInput(session, "select_gridvars", choices=colvars, selected=getDef(x, "gridvar"))
            if(!is.null(getDef(x, "assay"))) updateCheckboxInput(session, "hm_scale", value=!grepl("FC$",getDef(x, "assay")))
            return(x)
        }
        ...
        output$fileout <- renderText({
            ...
            tout <- "Loaded: "
            if(!is.null(SE()@metadata$title)) tout <- paste(tout, SE()@metadata$title)
        })
        ...
        output$gene_plot <- renderPlot({
            ...
            if(input$select_plotpoints) p <- p + geom_point(position = position_jitterdodge())
